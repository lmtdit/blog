...
@@css_head
```
index.html文件作为主模板来生成最终的静态demo，在这个过程中通过模板引擎向子模板传递参数，当需要构建不同的页面时，传递不同的参数
```
@@include("./_global/head.html",{
    "title":"首页",
    "css_head":"common,index"
...
可将静态的demo构建机制作为后端模板的构建机制，直接在开发过程中解决资源的定位问题，并对html模板进行压缩优化。
...
事实上，只要服务端存在view层并且都需要引用前端静态资源（css、js和img），不管该模板层由谁来需要维护，都存在上述的问题，前后端就无法彻底解耦。
